...
import { MARGIN_BOTTOM, GAP, PADDING, VERTICAL_SPACING } from '../constants/spacing'
...
export const LearningVelocityChart: React.FC<LearningVelocityChartProps> = ({ data, loading }) => {
  ...
  const maxReviewed = !loading && data.length > 0 ? Math.max(...data.map(d => d.wordsReviewed), 1) : 1
  const maxMastered = !loading && data.length > 0 ? Math.max(...data.map(d => d.wordsMastered), 1) : 1
  const maxValue = Math.max(maxReviewed, maxMastered, 10)

  // Calculate overall trends
  const totalReviewed = !loading && data.length > 0 ? data.reduce((sum, d) => sum + d.wordsReviewed, 0) : 0
  const totalMastered = !loading && data.length > 0 ? data.reduce((sum, d) => sum + d.wordsMastered, 0) : 0
  const averageAccuracy = !loading && data.length > 0 ? data.reduce((sum, d) => sum + d.accuracy, 0) / data.length : 0
